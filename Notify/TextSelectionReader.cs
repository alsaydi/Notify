//Thank you https://gist.github.com/RobThree/4943725
using System;
...
public class TextSelectionReader
{
    ...
    private string GetTextFromWin32Api()
    {
        ...
        Debug.WriteLine($"Control text length == {textlength}");

        //Have any text at all?
        if (textlength == 0)
        {
            Debug.WriteLine($"{nameof(GetTextFromWin32Api)} - failed to get selected text length.");
            return string.Empty;
        }
        ...
        Debug.WriteLine($"Selected text starts at {selstart} and ends at {selend}");
        ...
        sb.Clear();
        if ((selectedText.Length > 0) && (selend - selstart > 0) && (selstart < selectedText.Length) && (selend < selectedText.Length))
        {
            finalResult = selectedText.Substring(selstart, selend - selstart);
    ...
    /// fails to retrieve the text.
    /// </returns>
    [Obsolete("do not mess with the clipboard", true)]
